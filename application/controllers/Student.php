...
class Student extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        $last_students = $this->student_model->get_last_std();
        $currentRollNumber = ltrim($last_students->roll_no,"S-07-");
        $numlength=strlen($currentRollNumber);//        var_dump($total_students);
//        die();
        if ($numlength == 1) {
            $number = $currentRollNumber + 1;
            $student_number = "000" . $number;
        } elseif ($numlength == 2) {
            $number = $currentRollNumber + 1;
            $student_number = "00" . $number;

        } elseif ($numlength == 3) {
            $number = $currentRollNumber + 1;
            $student_number = "0" . $number;
        ...
        $data["roll_no"] = "S-07-" . $student_number;
            ...
            if ($insert) {
                ...
                $stdTuitionFee=0;
                    foreach ($feeTypeIds as $feeTypeId)
                    {
                        ...
                        {
                            $stdTuitionFee=$balance;
                            $last_total_fee=$last_total_fee-$balance;
                        }else
                        {
                            $feeDetail[]=array(
                                'feeTypeID'=>$feeTypeId,
                ...
                if($fees_discount > 0)
                {
                    $_discountRate=$fees_discount/100;
                    $_discountPrice=$_discountRate*$total_fee;
                    $last_amount=$total_fee-$_discountPrice;
                }else{
                    $last_amount=$total_fee;
                }
                $STDFeeDataArray=array(
                    'std_id'=>$insert_id,
                    'std_fee_case'=>$fee_case,
                    'std_tuition_fee'=>$stdTuitionFee,
                );
                $STDFeeID= $this->Shared_model->insert($STDFeeDataArray,"tbl_student_fee");
                if($fee_case==2)
                {
                    $month=$this->getFeeMonth(date('m'));
                }else{
                    $month=date('m');
                }
                $feeData=array(
              ...
              $discountDataArray=array(
                  ...
                  "discount_type"=>'multi',
                  "applied_at"=>'admission',
                  "applied_on"=>'multi',
                  "discount_rate"=>$discountDetailsEncoded,
              );
                $voucherDataArray = array(
                    'fee_detail_id' =>$feeDetailId,
                    'challan_no' => rand(000, 999),
                    'due_date' =>$due_date,
                    'issue_date' =>$issue_date,
                    'stuck_off_date' =>$due_date,
                    'month' =>$month,
                    'year' =>date('Y'),
                    'expire' =>1, /* 0 For Yes/1 For No*/
                );
                $voucherID= $this->Shared_model->insert($voucherDataArray,"tbl_vouchers");
                $this->Shared_model->insert($discountDataArray,"tbl_fee_discounts");
                if ($sibling_id > 0) {
...
//                            fee_case tuition
                            $stdFeeCase=$student_data[$i]["fee_case"];
                            $stdTuitionFee=$student_data[$i]["tuition"];
                            unset($student_data[$i]["tuition"]);
                            unset($student_data[$i]["fee_case"]);
    ...
    public function edit($id)
    {
        ...
        $data['sch_setting']        = $this->sch_setting_detail;
        $feeType_result            = $this->studentfee_model->getFeeType();
        $tutionFee            = $this->Shared_model->selectDataWhereSingle("tbl_student_fee",['std_id'=>$id]);
        $data['feeTypeList']       = $feeType_result;
        ...
        // $this->form_validation->set_rules('rte', $this->lang->line('rtl'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('guardian_phone', $this->lang->line('guardian_phone'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('tutionFee', 'Tuition Fee', 'trim|required|xss_clean');
        $this->form_validation->set_rules(
            'roll_no', $this->lang->line('roll_no'), array('trim',
                array('check_exists', array($this->student_model, 'valid_student_roll')),
            )
...
//            $feeTypeIds          = $this->input->post('feeType');
            $fee_case            = $this->input->post('fee_case');
            $stdTuitionFee            = $this->input->post('tutionFee');
//            $due_date            = $this->input->post('due_date');
//            $issue_date            = $this->input->post('issue_date');
//            $last_total_fee            = $this->input->post('total_fee');
            //            *****************Fee Module**********
            ...
            $checkExsists=$this->Shared_model->selectDataWhereSingle("tbl_student_fee",['std_id'=>$id]);
            if(!empty($checkExsists))
            {
                $updateFee=$this->Shared_model->updateData("tbl_student_fee",['std_id'=>$id],$STDFeeDataArray);
            }else{
                $STDFeeID= $this->Shared_model->insert($STDFeeDataArray,"tbl_student_fee");
...
//                'std_tuition_fee'=>$stdTuitionFee,
//            );
//            $STDFeeID= $this->Shared_model->updateData("tbl_student_fee",['std_id'=>$id],$STDFeeDataArray);
//            if($fee_case==2)
//            {
//                $month=$this->getFeeMonth(date('m'));
//            }else{
...
//            $this->Shared_model->insert($discountDataArray,"tbl_fee_discounts");
    ...
    {
        $data['title']       = 'Facebook Profile';
         $this->load->view('layout/header', $data);
        $this->load->view('facebook/facebook_page', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function games()
    {
        $data['title']       = 'Games';
        $this->load->view('layout/header', $data);
        $this->load->view('Games/web-games', $data);
        $this->load->view('layout/footer', $data);

    }

    public function digital_learning()
    {
        $data['title']       = 'Digital Learning';
        $this->load->view('layout/header', $data);
        $this->load->view('digital_learning/videos', $data);
        $this->load->view('layout/footer', $data);
    }
    public function youtubeVideos()
        {
            ...
            $data['title']       = 'Digital Learning';
            $this->load->view('layout/header', $data);
            $this->load->view('youtube_videos/index', $data);
            $this->load->view('layout/footer', $data);
        }
