    ...
    public function create()
    {
     ...
     $this->form_validation->set_rules('submit_date', $this->lang->line('submission_date'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($section_id=="all")
            {
                $allSections = $this->section_model->getClassBySection($this->input->post("modal_class_id"));
                foreach ($allSections as $section)
                {
                    ...
                    $ids[] = $this->homework_model->add($data);
                ...
                foreach ($ids as $s_id)
                {
                    $upload_data = array('id' => $s_id, 'document' => $document);
                    $this->homework_model->add($upload_data);
                }
                if ($record_id == 0) {
                    foreach ($ids as $mail_id)
                    {
                        $homework_detail = $this->homework_model->get($mail_id);
