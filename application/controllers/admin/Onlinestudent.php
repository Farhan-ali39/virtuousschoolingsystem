...
//        $student_result = $this->onlinestudent_model->get(null,$carray);
        $student_result = $this->onlinestudent_model->getOnlineStudents();
        $data['studentlist'] = $student_result;

        foreach ($student_result as $item)
        {
             $this->Shared_model->updateData("online_admissions_requests",['id'=>$item['main_id']],['notify_status'=>1]);

        }

        $this->load->view('layout/header', $data);
//        $this->load->view('admin/onlinestudent/studentList', $data);
        $this->load->view('admin/onlinestudent/onlinestudentList', $data);
    ...
    public function followUp()
    {
        ...
        $data     = $this->Shared_model->updateData("online_admissions_requests",['id'=>$request_id],$dataArray);
        $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">'.$this->lang->line('update_message').'</div>');
        redirect('admin/onlinestudent');
    ...
    public function deleteRequest($id)
    {
        ...
        if (!empty($id)) {
            $data     = $this->Shared_model->deleteData("online_admissions_requests",['id'=>$id]);
