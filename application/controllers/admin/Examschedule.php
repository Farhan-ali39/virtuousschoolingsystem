...
class ExamSchedule extends Admin_Controller {
    ...
    function index() {
        if (!$this->rbac->hasPrivilege('exam_publish', 'can_view')) {
            access_denied();
        }
    ...
    function create() {
        if (!$this->rbac->hasPrivilege('exam_publish', 'can_add')) {
            access_denied();
        }
...
//        $this->form_validation->set_rules('section_id', $this->lang->line('section'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->input->post('save_exam') == "save_exam") {
                ...
                {
                    $i = $this->input->post('i_'.$subjectId);
                    foreach ($i as $key2 => $value) {
                            ...
                            $data = array(
                                ...
                                'exam_id' => $this->input->post('exam_id'),
                                'date_of_exam' => date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('date_' . $subjectId))),
                                'start_to' => $this->input->post('stime_' . $subjectId),
                                'end_from' => $this->input->post('etime_' . $subjectId),
                                'room_no' => $this->input->post('room_' . $subjectId),
                                'full_marks' => $this->input->post('fmark_' . $subjectId),
                                'passing_marks' => $this->input->post('pmarks_' . $subjectId)
                            );
    ...
    function edit($id) {
        if (!$this->rbac->hasPrivilege('exam_publish', 'can_edit')) {
            access_denied();
        }
