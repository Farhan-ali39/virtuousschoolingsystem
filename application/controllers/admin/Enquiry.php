...
class Enquiry extends Admin_Controller {
    ...
    public function index() {
        ...
        foreach ($enquiry_list as $key => $value) {
            ...
            $feeDetails = $this->Shared_model->selectDataWhereSingle("tbl_temparory_fee_details",array('enquiry_id'=>$value["id"]));
            if(!empty($feeDetails))
            {
                $enquiry_list[$key]["feestatus"] = 1;
                $enquiry_list[$key]["feeDetailId"] = $feeDetails->fee_detail_id;
            }else{
                $enquiry_list[$key]["feestatus"] = 0;
            }
 ...
 public function follow_up($enquiry_id, $status) {
    ...
    function searchByVoucher()
    {
        ...
        if (isset($_POST["search"])) {
            ...
            $enquiry_list = $this->enquiry_model->searchByVoucher($voucher_no);
            foreach ($enquiry_list as $key => $value) {
