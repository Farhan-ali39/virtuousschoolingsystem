...
class Content extends Admin_Controller {
    ...
    function index() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($section_id=="all" && $classes!="")
            {
                $allSections = $this->section_model->getClassBySection($classes);
                foreach ($allSections as $section)
                {
                    $class_sec_id=$this->Shared_model->selectDataWhereSingle("class_sections",array('class_id'=>$classes,"section_id"=>$section['section_id']));
                    if(!empty($class_sec_id))
                    {
                        $data = array(
                        ...
                        $insert_ids[] = $this->content_model->add($data, $content_for);

                    }
                }
                $insert_id=$insert_ids[0];
                if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                    $fileInfo = pathinfo($_FILES["file"]["name"]);
                    $img_name = $insert_id . '.' . $fileInfo['extension'];
                    move_uploaded_file($_FILES["file"]["tmp_name"], "./uploads/school_content/material/" . $img_name);
                   foreach ($insert_ids as $insertId)
                   {
                       $data_img = array('id' => $insertId, 'file' => 'uploads/school_content/material/' . $img_name);
