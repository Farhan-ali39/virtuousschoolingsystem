...
class Question extends Admin_Controller
{
    ...
    public function index()
    {
       ...
       if (!$this->rbac->hasPrivilege('question_bank', 'can_view')) {
        ...
        $questionOpt          = $this->customlib->getQuesOption();
        $data['questionOpt']  = $questionOpt;
        $exam_result = $this->Shared_model->selectAll('onlineexam');
 ...
 if (!$this->rbac->hasPrivilege('question_bank', 'can_delete')) {
    ...
    public function getSubjects()
    {
        ...
        $examSchedule = $this->teachersubject_model->getSubjectsByClsandSection($class_id, $section_id);
         $subjectOption='';
        $subjectOption.='<option>Select Subject</option>';
        foreach ($examSchedule as $item)
        {
            $subjectOption.='<option value="'.$item['subject_id'].'">'.$item['name'].'</option>';

        }
        echo json_encode(array('optionList'=>$subjectOption));
