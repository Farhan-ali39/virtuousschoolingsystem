...
//        $this->form_validation->set_rules('name', $this->lang->line('subject_name'), 'trim|required|xss_clean|callback__check_name_exists');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $extra_array = array();
            if(isset($_POST['extra_assessment_0']))
            {


                for ($i=0;$i<$this->input->post('total_extra');$i++)
                {
                    array_push($extra_array, $this->input->post('extra_assessment_'.$i));
                }

            }

            $extra_encoded = json_encode($extra_array);
            $data = array(
    ...
    function edit($id) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(isset($_POST['extra_assessment_0']))
            {
                ...
                for ($i=0;$i<$this->input->post('total_extra');$i++)
                {
                    if(isset($_POST['extra_assessment_'.$i]))
                    {
                        array_push($extra_array, $this->input->post('extra_assessment_'.$i));
