...
class Media extends Student_Controller {
    ...
    function getYoutubeVideos() {
        ...
        if (!empty($result)) {
            ...
            foreach ($result as $res_key => $res_value) {
                ...
                $div = $this->generateDivForYoutubeVideos($res_value, $is_gallery);
    ...
    function getRecordedClasses() {
        ...
        $config = array();
        $config["base_url"] = "#";
        $config["total_rows"] = $this->Classschedule_model->count_all($keyword, $file_type,$class_id,$section_id);
        $config["per_page"] = 60;
        $config["uri_segment"] = 5;
        $config["use_page_numbers"] = TRUE;
        $config["full_tag_open"] = '<ul class="pagination">';
        ...
        $page = $this->uri->segment(4);
        $start = ($page - 1) * $config["per_page"];
        $result = $this->Classschedule_model->fetch_details($config["per_page"], $start, $keyword, $file_type,$class_id,$section_id);
        $img_data = array();
        $check_empty = 0;
        if (!empty($result)) {
            $check_empty = 1;
            foreach ($result as $res_key => $res_value) {
                ...
                $div = $this->generateDivForRecordedClass($res_value, $is_gallery);
...
//================
     ...
     function generateDivForYoutubeVideos($result, $is_gallery) {
...
//================
    ...
    function generateDivForRecordedClass($result, $is_gallery) {
...
//==============
        ...
        if (!$is_gallery) {
            $output .= "<div class='overlay3'>";
            $output .= "<a href='".base_url('user/user/recorded_class/').$result->id."' class='uploadcheckbtn' data-record_id='" . $result->id . "'   data-image='" . $file . "' data-source='" . $file_src . "' data-media_name='" . $result->img_name . "' data-media_size='" . $result->file_size . "' data-media_type='" . $result->file_type . "'><i class='fa fa-navicon'></i></a>";
            $output .= "<p class='processing'>Processing...</p>";
            $output .= "</div>";
        }
        if ($is_video == 1) {
