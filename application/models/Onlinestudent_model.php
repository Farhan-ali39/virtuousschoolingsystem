...
//===============End Parent ID===========
    ...
    {
        $this->db->select('online_admissions_requests.*,online_admissions_requests.id AS main_id,classes.*');
        $this->db->from('online_admissions_requests');
        $this->db->join('classes', 'online_admissions_requests.class_id = classes.id');
        $query = $this->db->get();
        return $query->result_array();
    }
