...
class Content_model extends MY_Model {
    ...
    public function getContentByRole($id = null, $role = null) {
        $query = "SELECT contents.*,(select GROUP_CONCAT(role) FROM content_for WHERE content_id=contents.id) as role,class_sections.id as `class_section_id`,classes.class,sections.section  FROM `content_for`  INNER JOIN contents on contents.id=content_for.content_id left JOIN class_sections on class_sections.id=contents.cls_sec_id left join classes on classes.id=contents.class_id LEFT JOIN sections on sections.id=contents.cls_sec_id WHERE (role='student' and created_by='" . $id . "' ) or (created_by=0 and role='" . $role . "')  GROUP by contents.id";
//        $query = "SELECT contents.*,(select GROUP_CONCAT(role) FROM content_for WHERE content_id=contents.id) as role,classes.class,sections.section  FROM `content_for`  INNER JOIN contents on contents.id=content_for.content_id left join classes on classes.id=contents.class_id LEFT JOIN sections on sections.id=contents.cls_sec_id WHERE (role='student' and created_by='" . $id . "' ) or (created_by=0 and role='" . $role . "')  GROUP by contents.id";
