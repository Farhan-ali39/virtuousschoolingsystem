...
class Staff_model extends MY_Model
{
    ...
    public function getLeaves($month, $year, $staff_id)
    {

//        $query1 = $this->db->select('*')->where(array('month(date)' => $month, 'year(date)' => $year, 'staff_id' => $staff_id, 'status' => 'approve'))->get("staff_leave_request");

        $query1 = $this->db->select('staff_leave_request.*,leave_types.*')->where(array('month(date)' => $month, 'year(date)' => $year, 'staff_id' => $staff_id, 'status' => 'approve'))->join("leave_types", "staff_leave_request.leave_type_id = leave_types.id")->get("staff_leave_request");
