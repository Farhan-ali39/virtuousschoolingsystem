...
class enquiry_model extends MY_Model {
    ...
    function searchByVoucher($voucher_no)
    {
        $this->db->select('tbl_vouchers.*,tbl_temparory_fee_details.*,enquiry.*')->from('tbl_vouchers');
        $this->db->join('tbl_temparory_fee_details', 'tbl_vouchers.temp_fee_detail_id = tbl_temparory_fee_details.fee_detail_id');
        $this->db->join('enquiry', 'tbl_temparory_fee_details.enquiry_id = enquiry.id');
        $this->db->like('tbl_vouchers.challan_no', $voucher_no);
        $query = $this->db->get();
        // echo $this->db->last_query();die;
        return $query->result_array();
