...
class Student_model extends MY_Model
{
    ...
    {
        $this->db->select('student_session.transport_fees,tbl_student_fee.*,students.app_key,students.vehroute_id,vehicle_routes.route_id,vehicle_routes.vehicle_id,transport_route.route_title,vehicles.vehicle_no,hostel_rooms.room_no,vehicles.driver_name,vehicles.driver_contact,hostel.id as `hostel_id`,hostel.hostel_name,room_types.id as `room_type_id`,room_types.room_type ,students.hostel_room_id,student_session.id as `student_session_id`,student_session.fees_discount,,student_session.fee_case,classes.id AS `class_id`,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode , students.note, students.religion, students.cast, school_houses.house_name,   students.dob ,students.current_address, students.previous_school,
            students.guardian_is,students.parent_id,
...
//        $this->db->where('student_session.session_id', $this->current_session);
    ...
    public function searchFullText($searchterm, $carray = null)
    {
        ...
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
        $this->db->join('tbl_student_fee', 'tbl_student_fee.std_id = students.id');
        $this->db->join('tbl_student_fee_detail', 'tbl_student_fee.std_fee_id = tbl_student_fee_detail.std_fee_id','left' );
        $this->db->join('tbl_vouchers', 'tbl_student_fee_detail.fee_detail_id = tbl_vouchers.fee_detail_id','left' );
        $this->db->where('student_session.session_id', $this->current_session);
        $this->db->where('students.is_active', 'yes');
        $this->db->group_start();
        ...
        $this->db->or_like('tbl_vouchers.challan_no', $searchterm);
        $this->db->group_end();
        $this->db->order_by('students.id');
 ...
 if (($userdata["role_id"] == 2) && ($userdata["class_teacher"] == "yes")) {
    ...
    public function remove($id)
    {
        ...
        $this->db->delete('tbl_student_fee');
    ...
    public function searchByClassAndFeeCase(  $feeCase = null,$section_id = null,$class_id = null,$session=false)
    {
        ...
        $this->db->select('classes.id AS `class_id`,student_session.id as student_session_id,students.id,students.id as student_main_id,classes.class,sections.id AS `section_id`,sections.section,tbl_student_fee.*,students.id,students.admission_no ,students.roll_no , students.roll_no,students.admission_date,students.firstname,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode ,     students.religion,     students.dob ,students.current_address,    students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.rte,students.gender,users.id as `user_tbl_id`,users.username,users.password as `user_tbl_password`,users.is_active as `user_tbl_active`')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('tbl_student_fee', 'tbl_student_fee.std_id = students.id');
        ...
        if ($feeCase != null) {
            $this->db->where('tbl_student_fee.std_fee_case', $feeCase);
        }
    ...
    function searchByMonthAndFeeCase($month,$fee_year,$feeCase)
    {
        ...
        $field_variable = implode(',', $field_var_array);
//        $this->db->select('tbl_student_fee.*,tbl_student_fee_detail.*,tbl_vouchers.*,classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode ,     students.religion,     students.dob ,students.current_address,    students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.app_key,students.parent_app_key,students.rte,students.gender,' . $field_variable)->from('students');
//        $this->db->join('student_session', 'student_session.student_id = students.id');
//        $this->db->join('classes', 'student_session.class_id = classes.id');
//        $this->db->join('sections', 'sections.id = student_session.section_id');
//        $this->db->join('categories', 'students.category_id = categories.id', 'left');
//        $this->db->join('tbl_student_fee', 'tbl_student_fee.std_id = students.id');
////        $this->db->join('students', 'tbl_student_fee.std_id = students.id');
//        $this->db->join('tbl_student_fee_detail', 'tbl_student_fee_detail.std_fee_id = tbl_student_fee.std_fee_id');
//        $this->db->join('tbl_vouchers', 'tbl_student_fee_detail.fee_detail_id = tbl_vouchers.fee_detail_id');
//        $this->db->where('student_session.session_id', $this->current_session);
//        $this->db->where('students.is_active', "yes");


        $this->db->select('tbl_student_fee.*,tbl_student_fee_detail.*,tbl_vouchers.*,classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode ,     students.religion,     students.dob ,students.current_address,    students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.app_key,students.parent_app_key,students.rte,students.gender,' . $field_variable)->from('tbl_student_fee');
        $this->db->join('students', 'tbl_student_fee.std_id = students.id');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
...
//        $this->db->where('student_session.session_id', $this->current_session);
        $this->db->where('students.is_active', "yes");
        if ($month != null) {
            $this->db->where('tbl_student_fee_detail.fee_month', $month);
            $this->db->where('tbl_student_fee_detail.fee_year', $fee_year);
//            $this->db->where('tbl_vouchers.month', $month);
//            $this->db->where('tbl_vouchers.fee_year', $year);
        }
        if ($feeCase != null) {
//            $this->db->where('student_session.fee_case', $feeCase);
            $this->db->where('tbl_student_fee.std_fee_case', $feeCase);
        }
        //$this->db->order_by('students.id');
        $this->db->order_by('students.admission_no', 'asc');
    ...
    {
        $this->db->select('tbl_student_fee.*,tbl_student_fee_detail.*')->from('tbl_student_fee');
        $this->db->join('tbl_student_fee_detail', 'tbl_student_fee_detail.std_fee_id = tbl_student_fee.std_fee_id');
        if ($id != null) {
             $this->db->where('tbl_student_fee.std_id', $id);
        }
        $this->db->order_by('tbl_student_fee_detail.fee_detail_id', 'DESC');

        $query = $this->db->get();

        return $query->result_array();
    }
    function getFeeAssignedStudents($month,$feeCase,$fee_year)
    {
        $session_current   = substr($this->setting_model->getCurrentSessionName(),'5');
        $currentYear=substr(date('Y'),'0','2');
...
//        $this->db->where('students.is_active', "yes");
        ...
        if ($month != null) {
            $this->db->where('tbl_student_fee_detail.fee_month', $month);
//            $this->db->where('tbl_student_fee_detail.fee_year', $year);
//            $this->db->where('tbl_vouchers.month', $month);
//            $this->db->where('tbl_vouchers.fee_year', $year);
        }
        if ($fee_year != null) {
...
//            $this->db->where('student_session.fee_case', $feeCase);
    ...
    function get_last_std()
    {
        ...
        return $result;
//        $query = $this->db->query("SELECT * FROM students ORDER BY id DESC LIMIT 1");
//        $result = $query->row();
//        return $result;
    }
}
