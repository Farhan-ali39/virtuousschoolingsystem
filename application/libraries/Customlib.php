...
class Customlib
{
    ...
    public function __construct()
    {
        ...
        $this->CI->load->model('Shared_model', '', true);
    }
    ...
    public function getAdminSessionFirstName()
    {
        $student_session = $this->CI->session->userdata('admin');
        $username        = $student_session['firstname'];
        return $username;
    }
...
//staff_statusmessage
    ...
    public function mychatnotification() {
        $chat_user = $this->CI->chatuser_model->getMyID($this->getStaffID(), 'staff');
        $notifications = array();
        if (!empty($chat_user)) {
            $notifications = $this->CI->chatuser_model->getChatNotification($chat_user->id);
        }

        return $notifications;
    }
    public function mychatnotificationstd()
    {
        $student_id    = $this->getStudentSessionUserID();
        $chat_user     = $this->CI->chatuser_model->getMyID($student_id, 'student');
    ...
    public function newAdmmissionNotifications()
    {
        $lists     = $this->CI->Shared_model->selectDataWhereMultiple("online_admissions_requests",['notify_status'=>0]);
