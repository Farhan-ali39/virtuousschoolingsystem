...
class Mailgateway
{
    ...
    public function sentAddFeeMail($detail, $template)
    {
        ...
        $msg     = $this->feeDepositContent($detail, $template);
    ...
    public function feeDepositContent($data, $template)
    {
        $currency_symbol      = $this->sch_setting[0]['currency_symbol'];
        $school_name          = $this->sch_setting[0]['name'];

        $msg_content['firstname']      = $data['firstname'];
        $msg_content['lastname']       = $data['lastname'];
        $msg_content['class']          = $data['class'];
        $msg_content['school_name']    =$school_name;
        $msg_content['fee_amount']     = $currency_symbol . $data['fee_amount'];
