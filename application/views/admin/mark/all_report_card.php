...
if($school_id==1 && $examType==1) {
    ...
    foreach ($student_ids as $student_id) {
        $student_exam_result = $this->examresult_model->genrateSinglereport($exam_sec_id, $student_id);
        $student_assesment_data = $this->examresult_model->getAssessmentMarks($student_id);
            ...
            <table style="margin-top: .3in; width: 100%">
                <tr style="text-align: left;">
                    ...
                    <th style="padding-left: 155px;line-height: 5px;"><h2>Student's Monthly Progress Report</h2>
                        <!--                    <p>PECHS Campus</p><p><b>Assessment 2019-20</b></p><p><b>-->
                        <?//=$student_exam_result[0]?><!-- Result card</b></p>-->
                    </th>
                </tr>
            </table>
            <div style="float: left; width: 350px; border: 2px solid;">
...
<!--                    . " " . $student_exam_result[0]->lastname-->
            ...
            <table id="customers" style="width:100%">
                ...
                //            $total_marks=0;
                foreach ($student_exam_result as $value)if($value->name!="Creative & Expressive Arts" ) {
                      $ci =& get_instance();
                    $ci->load->model('Examresult_model');
...
//                $total_marks=$total_marks+$value->full_marks;
            ...
            <p>Please tally the grading with the chart below and find out the subject teacher’s impressions regarding
                your child’s performance in each subject. The class teacher’s comments are given for your information.
            </p>
            <center><h3 style="display: -webkit-inline-box; display: inline-block;">TEACHER’S IMPRESSION CHART</h3>
            </center>
            <table id="customers" style="width:100%">
                <tr>
                    <th style="text-align: center;">Grade</th>
            ...
            <table id="customers" style="margin-top: 2%">
                ...
                <tr>
                    <td><span style="margin-left: 50px; margin-right: 50px">A-Excellent</span> <span
                            style="margin-left: 50px; margin-right: 50px">B-Good</span><span
                            style="margin-left: 50px; margin-right: 50px">C-Satisfactory</span><span
                            style="margin-left: 50px; margin-right: 50px">D-Needs Improvement</span></td>
...
<body>
    ...
    ?>
    <div class="wrapper">
        <table style="  width: 100%;margin-top: .3in">
            <tr style="text-align: left;">
                <th style="width: 100px;"><img src="<?=base_url('uploads/')?>logo.jpg"></th>
                <th style="padding-left: 155px; "><h2 style="line-height: 5px;">Leadership School</h2><p style="margin-left: 25px;line-height: 3px;">PECHS Campus</p><p style="margin-left: 25px;margin-top: 20px; line-height: 0px;"><b><?=$examData->name?> <?=$this->setting_model->getCurrentSessionName()?></b></p><p><b><?=$student_exam_result[0]->class?> Assessment Report</b></p></th>
            </tr>
        </table>
...
<!--                ." ".$student_exam_result[0]->lastname-->
        ...
        unset($exam_result_ids);
        $subjectIds = [];
        foreach ($student_exam_result as $value)
        {
            if(in_array($value->subject_id , $subjectIds))
                {
                    continue;
                }
           ...
           if(!empty($subject_extras_grades))
           {
               $encoded_data= json_decode($subject_extras_grades->extra_grades);
                ...
                if($numberOfrows!=0)
                {
                    ...
                    for ($i=0;$i<$numberOfrows;$i++)
                    {
                        ...
                        <tr>
                            ...
                            {
                                if(isset($subject_extras_data[$j])  )
                                {
                                    if($subject_extras_data[$j]!="Addition" && $subject_extras_data[$j]!="Number in words")
                                    {
                                                                           ?>
                                    <td><?=ucfirst($subject_extras_data[$j])?>  </td>
                                    <td style="text-align: center"><?=(isset($encoded_data[$j])?ucfirst($encoded_data[$j]):"")?></td>
                                    <?php
           ...
           {
               $exam_result_ids[]=array('subject_name'=>$value->name,'result_id'=>$value->exam_result_id);
           }
        }
        if(!empty($exam_result_ids))
        {
            $numberOfSubrows=0;
            $sub_count=1;
            $subUpto=3;
            ...
            $numberOfSubrows= ceil(count($exam_result_ids)/3);
            for($i=1;$i<=$numberOfSubrows;$i++) {
                echo "<tr>";
                for($j=1;$j<=$subUpto;$j++) {
                    $subjectGrade = $this->Shared_model->selectDataWhereSingle("exam_results",array('id'=>$exam_result_ids[$main_index]['result_id']))->get_grades;
                    ?>

                        <td style="width: 30%;background-color: #464a46;color: white">
<!--                            -->
                             <?=ucfirst($exam_result_ids[$main_index]['subject_name'])?>
                        </td>
                    <td style="text-align: center;width: 3.33%">
                        <?=$subjectGrade?>
                    </td>
        ...
        <table id="customers" style="margin-top: 1%" >
            <tr>
...
<!--            </div>-->
            ...
            <table id="customers" style="width:100%">
                ...
                <tr style="background-color: #464a46; color: white;">
                    ...
                    <td colspan="3" style="border: 1px solid #f2fff2;"><?= round($total_obtain_marks / $total_marks * 100,2) ?>
                        %
                    </td>
...
elseif($school_id==2 && $examType==2) {
    ...
    foreach ($student_ids as $student_id) {
        $totalAssesmentMarks= $first_total_marks+$second_total_marks;
        ...
        foreach ($student_exam_result as $value) {
            ...
            {
                $get_marks=$value->get_marks;
                $total_marks = round((($first_marks[$student_id][$index]+$second_marks[$student_id][$index]+$value->get_marks)/($value->full_marks+$totalAssesmentMarks))*100,1) ;
                $totalForPercentage=$totalForPercentage+$total_marks;

            }
//            $total_marks = (((($first_marks[$student_id][$index]/$first_total_marks)*100) + (($second_marks[$student_id][$index]/$second_total_marks)*100))/2 + (($value->get_marks/$value->full_marks)*100)/2) ;
...
//        echo "<pre>";
//        var_dump($totalForPercentage);
//        die();
        if($totalForPercentage!=0)
        {
                    $ans=round($totalForPercentage/$numberOfSubject,2);
            $overAll[]=$ans;
...
//            die();
        ...
        <div class="wrapper">
            <table style="margin-top: .3in; width: 100%">
                <tr style="text-align: left;">
                    <th style="width: 100px;"><img src="<?= base_url('uploads/') ?>logo.jpg"></th>
                    <th style="padding-left: 12px;line-height: 5px;"><h2>Leadership School</h2>
                        <p>PECHS Campus</p>
                        <p><b><?=$examData->name?>   <?=$this->setting_model->getCurrentSessionName()?></b></p>
                        <p><b><?= $student_exam_result[0]->class ?>- Result card</b></p></th>
                </tr>
            </table>
            <div style="height: 65px; float: left; width: 350px; border: 2px solid;">
                <div style="text-align: right;width: 30%; float: left;">
...
<!--                    . " " . $student_exam_result[0]->lastname-->
            ...
            <br><br><br><br><br>
                ...
                <div class="col-md-12">
<!--                    <center>-->
                        <h2 style=" text-align: center">ACADEMIC PERFORMANCE</h2>
<!--                    </center>-->
            ...
            <table id="customers" style="width:100%">
                <tr>
                    ...
                    <th>Subject</th>
                    <th style="text-align: center">Term(50%)</th>
...
//                            die();
                foreach ($student_exam_result as $value) {
                    $numberOfSubject++;
                    $total_marks = 0;
//                    $total_marks =round((((($first_marks[$student_id][$index]/$first_total_marks)*100) + (($second_marks[$student_id][$index]/$second_total_marks)*100))/2 + (($value->get_marks/$value->full_marks)*100)/2),1);
                    $total_marks =round((($first_marks[$student_id][$index]+$second_marks[$student_id][$index]+$value->get_marks)/($value->full_marks+$totalAssesmentMarks))*100,1);
                    $totalForPercentage=$totalForPercentage+$total_marks;
...
<!--                            --><?//=round( ((($first_marks[$student_id][$index]/$first_total_marks)*100) + (($second_marks[$student_id][$index]/$second_total_marks)*100))/2,1) ?>
                            <?=$first_marks[$student_id][$index]+$second_marks[$student_id][$index]?>
                        </td>
                         <?php
                            if($value->attendence=="ABS")
                            {
                         ...
                         <td style="text-align: center"><?=$get_marks?></td>
                        <td style="text-align: center">
<!--                            --><?//=round( (((($first_marks[$student_id][$index]/$first_total_marks)*100) + (($second_marks[$student_id][$index]/$second_total_marks)*100))/2 + (($value->get_marks/$value->full_marks)*100)/2) ,1) ?>
                        <?=round((($first_marks[$student_id][$index]+$second_marks[$student_id][$index]+$value->get_marks)/($value->full_marks+$totalAssesmentMarks))*100,1)    ?>

                        </td>
                        <td style="text-align: center">
                            <?php
                            if ($total_marks >= 89.50 && $total_marks <= 100) {
                                $APlus++;
                                echo "A+";
                            } elseif ($total_marks >= 79.50 && $total_marks <= 89.40) {
                                echo "A";
                                $A++;
                            } elseif ($total_marks >= 69.50 && $total_marks <= 79.40) {
                                echo "B";
                                $B++;
                            } elseif ($total_marks >= 59.50 && $total_marks <= 69.40) {
                                echo "C";
                                $C++;
                            } elseif ($total_marks >= 0 && $total_marks <= 59.40) {
                                echo "D";
                                $D++;
                            }
                            ?>
                ...
                ?>
                    ...
                    <table id="customers" style="width:100%; margin-top: 30px; margin-bottom: 30px">
                        ...
                        <tr>
                            <td>OVERALL %</td>
                            <td><?=round($totalForPercentage/$numberOfSubject,2)?>%</td>
                            <td>OVERALL GRADE</td>
                            <td>
                                <?php
                            $overallMarksforGrade= round($totalForPercentage/$numberOfSubject,2);
                                
                            if ($overallMarksforGrade >= 89.50 && $overallMarksforGrade <= 100) {
                                 echo "A+";
                            } elseif ($overallMarksforGrade >= 79.50 && $overallMarksforGrade <= 89.40) {
                                echo "A";
                             } elseif ($overallMarksforGrade >= 69.50 && $overallMarksforGrade <= 79.40) {
                                echo "B";
                             } elseif ($overallMarksforGrade >= 59.50 && $overallMarksforGrade <= 69.40) {
                                echo "C";
                             } elseif ($overallMarksforGrade >= 0 && $overallMarksforGrade <= 59.40) {
                                echo "D";
                             }
                                ?>
...
<!--                            <td></td>-->
            ...
            <table id="customers" style="width:100%">
                ...
                <tr>
                    <td colspan="6"><span style="margin-left: 50px; margin-right: 50px">EX=Excellent</span> <span
                            style="margin-left: 50px; margin-right: 50px">G= Good</span><span
                            style="margin-left: 50px; margin-right: 50px">S= Satisfactory</span><span
                            style="margin-left: 50px; margin-right: 50px">N.I=Needs Improvement</span></td>
                </tr>
            </table>
