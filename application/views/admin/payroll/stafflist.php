                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select autofocus="" id="month_id" name="month" class="form-control" >
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select autofocus="" id="year_id" name="year" class="form-control" >
                                            <option value="select"><?php echo $this->lang->line('select'); ?></option>
...
<!--                                <button type="button" name="viewPayslipAll" style="margin-left: 15px" onclick="viewPayslipAll()" value="all" class="btn btn-primary ml-1 btn-sm pull-right checkbox-toggle">--><?php //echo $this->lang->line('view'); ?><!-- --><?php //echo $this->lang->line('payslip'); ?><!-- --><?php //echo $this->lang->line('selected'); ?><!--</button>-->


                                <button type="button" name="generatePayrollAll" onclick="payToAll()"  value="all" class="btn btn-primary btn-sm pull-right checkbox-toggle">Pay To <?php echo $this->lang->line('selected'); ?></button>

                                <button type="button" name="generatePayrollAll" style="margin-right: 15px" onclick="generatePayrollAll()" value="all" class="btn btn-primary ml-1 btn-sm pull-right checkbox-toggle"><?php echo $this->lang->line('generate'); ?> <?php echo $this->lang->line('payroll'); ?> <?php echo $this->lang->line('selected'); ?></button>
                            </div>
                        </div>
                        <div class="box-body table-responsive">

                            <div class="download_label"><?php echo $this->lang->line('staff'); ?> <?php echo $this->lang->line('list'); ?></div>
                            <form id="staffListForm" action="">
...
<div id="proceedtopayAll" class="modal fade " role="dialog">
    <div class="modal-dialog modal-dialog2 modal-lg" style="    width: 1200px;">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">Pay To All</h4>
            </div>
            <div class="modal-body">

                <div class="row">
                    <form role="form" id="payToAllForm" action="<?php echo site_url('') ?>">
...
?>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                            <button type="button" class="btn btn-primary   pull-right" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> Processing"> <?php echo $this->lang->line('save'); ?></button>
                        </div>
...
<script type="text/javascript">
    ...
    function generatePayrollAll() {
        ...
        var formData = $('#staffListForm').serializeArray();
        var month= $("#month_id").val();
        var  year= $("#year_id").val();
        if(month.length===0 || month=="select")
        {
            var message1='Please Select the month';
            errorMsg(message1);
        }else if(year.length===0 || year=="select")
        {
            var message2='Please Select the year';
            errorMsg(message2);
        }else if(formData.length===0)
        {
            var message3='Please Select the staff';
            errorMsg(message3);
        } else {
            $.ajax({
                url: base_url + 'admin/payroll/generatePayroll',
                type: 'POST',
                data: {
                    formData: formData,
                    month: month,
    ...
    function payToAll() {
        ...
        }else if(formData.length===0)
        {
            ...
            $.ajax({
                url: base_url + 'admin/payroll/payToAllModel',
                type: 'POST',
                data: {
                    formData: formData,
                    month: month,
    ...
    function viewPayslipAll() {

    }
    $(document).on('click', '.paytoAllBtn', function (e) {

        var $this = $(this);
        $this.button('loading');
        $.ajax({
            url: '<?php echo site_url("admin/payroll/paymentSuccessAll") ?>',
            type: 'post',
            data: $('#payToAllForm').serialize(),
            dataType: 'json',
