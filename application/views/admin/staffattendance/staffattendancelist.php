...
function is_leave($getStaffApprovedLeaves,$attendence_date)
{
    $flag=false;
    $leave_type_id=0;
    foreach ($getStaffApprovedLeaves as $approvedLeaves)
    {
        $leave_type_id=0;
        $leave_from=strtotime($approvedLeaves->leave_from);
        $leave_to=strtotime($approvedLeaves->leave_to);
        $current_date=strtotime($attendence_date);
        $array = array();
         for ($i = $leave_from; $i <= $leave_to; $i += 86400) {
             if($current_date==$i)
             {
                 $flag=true;
                 $leave_type_id=$approvedLeaves->leave_type_id;
             }
         }
   ...
   return array('status'=>$flag,'leave_type_id'=>$leave_type_id);
}
                                                ...
                                                foreach ($resultlist as $key => $value) {
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            $c = 1;
                                                            $count = 0;
                                                            $getStaffApprovedLeaves=$this->Shared_model->selectDataWhereMultiple("staff_leave_request",array('staff_id'=>$value["staff_id"],'status'=>'approve'));
                                                            $attendence_date=$date;
                                                            $isLeave= is_leave($getStaffApprovedLeaves,$attendence_date);
                                                            $disable="";
                                                            $present_checked="";
                                                            $absent_checked="";
                                                            if($isLeave['status'])
                                                            {
                                                                ...
                                                                $absent_checked="";
                                                                $disable="disabled";
                                                                $getLeaveType=$this->Shared_model->selectDataWhereSingle("leave_types",array('id'=>$isLeave['leave_type_id']));
                                                                if($getLeaveType->include_salary==0)
                                                                {
                                                                    $absent_checked="checked";
                                                                }elseif ($getLeaveType->include_salary==1)
                                                                {
                                                                ...
                                                                if ($type['key_value'] != "H") {
                                                                    ...
                                                                    if ($value["date"] != "xxx") {
                                                                        ...
                                                                        <div class="radio radio-info radio-inline">
                                                                            <input <?php if($type['id']==1 && $present_checked=="checked"){echo $present_checked ; $present_checked=""; }elseif ($type['id']==3 && $absent_checked=="checked"){echo  $absent_checked;$absent_checked="";}else{ echo $disable ;}  ?>      <?php if ($value['staff_attendance_type_id'] == $type['id']) echo "checked"; ?>  type="radio" id="attendencetype<?php echo $value['staff_id'] . "-" . $count; ?>" value="<?php echo $type['id'] ?>" name="attendencetype<?php echo $value['staff_id']; ?>" >
                                                                        ...
                                                                        <div class="radio radio-info radio-inline">
                                                                            <input <?php if($type['id']==1 && $present_checked=="checked"){echo $present_checked ; $present_checked=""; }elseif ($type['id']==3 && $absent_checked=="checked"){echo  $absent_checked;$absent_checked="";}else{ echo $disable ;}  ?>  <?php if (($c == 1) && ($resultlist[0]['staff_attendance_type_id'] != 5)) echo "checked"; ?> type="radio" id="attendencetype<?php echo $value['staff_id'] . "-" . $count; ?>" value="<?php echo $type['id'] ?>" name="attendencetype<?php echo $value['staff_id']; ?>" >
                                                                            <label for="attendencetype<?php echo $value['staff_id'] . "-" . $count; ?>"> 
                                                                                <?php echo $type['type']; ?> 
                                                                            </label>
